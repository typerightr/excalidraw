...
var getDefaultAppState = () => {
  return {
    ...
    lastStickyNoteBackgroundColor: COLOR_PALETTE.yellow[DEFAULT_ELEMENT_BACKGROUND_COLOR_INDEX],
...
var APP_STATE_STORAGE_CONF = /* @__PURE__ */ ((config) => config)({
  ...
  currentItemEndArrowhead: { browser: false, export: false, server: false },
  ...
  currentItemStartArrowhead: { browser: false, export: false, server: false },
...
import { LOCAL_FONT_PROTOCOL as LOCAL_FONT_PROTOCOL4 } from "@excalidraw/common";
var HelveticaFontFaces = [
  {
    uri: LOCAL_FONT_PROTOCOL4
  }
];
...
var GridDotColor = {
  [THEME3.LIGHT]: "#d1d1d1",
  [THEME3.DARK]: applyDarkModeFilter2("#d1d1d1")
};
var strokeDotGrid = (context, gridSize, scrollX, scrollY, theme, width, height, zoom) => {
...
var _renderStaticScene = ({
  ...
  visibleElements.filter((el) => isFrameLikeElement(el)).forEach((frame) => {
...
var renderSceneToSvg = (elements, elementsMap, rsvg, svgRoot, files, renderConfig) => {
  ...
  const ensureFrameShadowFilter = () => {
    ...
    if (!svgRoot.querySelector(`#${FRAME_SHADOW_FILTER_ID}`)) {
      ...
      const feDropShadow = svgRoot.ownerDocument.createElementNS(SVG_NS, "feDropShadow");
      feDropShadow.setAttribute("dx", String(FRAME_STYLE2.shadowOffsetX));
      feDropShadow.setAttribute("dy", String(FRAME_STYLE2.shadowOffsetY));
      feDropShadow.setAttribute(
        "stdDeviation",
        String(FRAME_STYLE2.shadowBlur / 2)
      );
  ...
  elements.forEach((element) => {
    if (!element.isDeleted && isFrameLikeElement2(element) && element.backgroundColor !== "transparent" && renderConfig.frameRendering?.enabled) {
...
var addFrameLabelsAsTextElements = (elements, opts) => {
  const nextElements = [];
  for (const element of elements) {
    if (isFrameLikeElement3(element)) {
  ...
  renderSceneToSvg(
    ...
    files || {},
    {
      ...
      embedsValidationStatus: renderEmbeddables ? new Map(
        elementsForRender.filter((element) => isFrameLikeElement3(element)).map((element) => [element.id, true])
      ) : /* @__PURE__ */ new Map(),
...
var parseFileContents = async (blob) => {
  ...
  if (blob.type === MIME_TYPES6.png) {
    try {
      return await (await import("./data/image-5NYQCMPO.js")).decodePngMetadata(blob);
    } catch (error) {
...
//# sourceMappingURL=chunk-QOIZBBOR.js.map
